...
final class AssociationConfigurator implements FieldConfiguratorInterface
{
    ...
    {
        return AssociationExtendedField::class === $field->getFieldFqcn();
    }
    ...
    public function configure(FieldDto $field, EntityDto $entityDto, AdminContext $context): void
    {
        ...
        $targetCrudControllerFqcn = $field->getCustomOption(AssociationExtendedField::OPTION_EMBEDDED_CRUD_FORM_CONTROLLER)
            ?? $context->getAdminControllers()->findCrudControllerByEntity($entityDto->getClassMetadata()->getAssociationTargetClass($propertyName));

        if (true === $field->getCustomOption(AssociationExtendedField::OPTION_RENDER_AS_EMBEDDED_FORM)) {
        ...
        $field->setCustomOption(AssociationExtendedField::OPTION_EMBEDDED_CRUD_FORM_CONTROLLER, $targetCrudControllerFqcn);

        if (AssociationExtendedField::WIDGET_AUTOCOMPLETE === $field->getCustomOption(AssociationExtendedField::OPTION_WIDGET)) {
        ...
        $field->setFormTypeOption('attr.data-ea-autocomplete-render-items-as-html', true === $field->getCustomOption(AssociationExtendedField::OPTION_ESCAPE_HTML_CONTENTS) ? 'false' : 'true');
        ...
        if (\count($propertyNameParts) > 1) {
            ...
            try {
                if (null !== $entityDto->getInstance()) {
                    ...
                    $field->setCustomOption(AssociationExtendedField::OPTION_RELATED_URL, $this->generateLinkToAssociatedEntity($targetCrudControllerFqcn, $relatedEntityDto));
        ...
        if (true === $field->getCustomOption(AssociationExtendedField::OPTION_AUTOCOMPLETE)) {
            $targetCrudControllerFqcn = $field->getCustomOption(AssociationExtendedField::OPTION_EMBEDDED_CRUD_FORM_CONTROLLER);
            if (null === $targetCrudControllerFqcn) {
            ...
            try {
                $autocompleteEndpointUrl = $this->adminUrlGenerator
                    ...
                    ->set(AssociationExtendedField::PARAM_AUTOCOMPLETE_CONTEXT, [
            ...
            $autocompleteCallback = $field->getCustomOption(AssociationExtendedField::OPTION_AUTOCOMPLETE_CALLBACK);
            $autocompleteTemplate = $field->getCustomOption(AssociationExtendedField::OPTION_AUTOCOMPLETE_TEMPLATE);
            ...
            $field->setFormTypeOptionIfNotSet('query_builder', static function (EntityRepository $repository) use ($field) {
                ...
                if (null !== $queryBuilderCallable = $field->getCustomOption(AssociationExtendedField::OPTION_QUERY_BUILDER_CALLABLE)) {
        ...
        foreach ($settableOptions as $option) {
            if ($val = $field->getCustomOptions()->get($option)) {
    ...
    {
        $field->setCustomOption(AssociationExtendedField::OPTION_DOCTRINE_ASSOCIATION_TYPE, 'toOne');
        ...
        try {
            $field->setCustomOption(AssociationExtendedField::OPTION_RELATED_URL, $this->generateLinkToAssociatedEntity($targetCrudControllerFqcn, $targetEntityDto));
        } catch (RouteNotFoundException $e) {
            // this may throw a "route not found" exception if the associated entity is not
            // accessible from this dashboard; do nothing in that case.
        }
    ...
    {
        $field->setCustomOption(AssociationExtendedField::OPTION_DOCTRINE_ASSOCIATION_TYPE, 'toMany');
    ...
    private function configureCrudForm(FieldDto $field, EntityDto $entityDto, string $propertyName, string $targetEntityFqcn, string $targetCrudControllerFqcn): void
    {
        ...
        if (null === $associatedEntity) {
            ...
            $targetCrudControllerPageName = $field->getCustomOption(AssociationExtendedField::OPTION_EMBEDDED_CRUD_FORM_NEW_PAGE_NAME) ?? Crud::PAGE_NEW;
            $crudPageName = Crud::PAGE_NEW;
        } else {
            $targetCrudControllerAction = Action::EDIT;
            $targetCrudControllerPageName = $field->getCustomOption(AssociationExtendedField::OPTION_EMBEDDED_CRUD_FORM_EDIT_PAGE_NAME) ?? Crud::PAGE_EDIT;
    ...
    {
        $preferredChoices = $field->getCustomOption(AssociationExtendedField::OPTION_PREFERRED_CHOICES);
